    ...
    <main className="min-h-screen bg-[#f5f5f5] text-[#111]">
      <section className="mx-auto max-w-[1440px] xl:px-6 xl:py-12">
        ...
        <div className="grid gap-0 xl:gap-8 xl:grid-cols-[1.1fr_0.9fr] xl:items-start">
          {/* Columna imagen — order-1 mobile, col-1 row-1 desktop */}
          <div className="order-1 xl:[grid-area:1/1/2/2] bg-white xl:rounded-[2rem] xl:border xl:border-black/8 xl:p-4 xl:shadow-[0_18px_40px_rgba(15,23,42,0.05)]">
            <ProductImageGallery
              key={producto.slug}
          ...
          <div className="order-2 xl:[grid-area:1/2/3/3] space-y-4 bg-white px-5 py-6 xl:rounded-[2rem] xl:border xl:border-black/8 xl:p-8 xl:shadow-[0_18px_40px_rgba(15,23,42,0.05)]">
            ...
            {producto.fichaTecnicaUrl && (
              <a
                href={`/api/download-pdf?url=${encodeURIComponent(producto.fichaTecnicaUrl)}&filename=${encodeURIComponent(`ficha-tecnica-${producto.slug}.pdf`)}`}
                className="inline-flex items-center gap-2 rounded-xl border border-black/10 px-5 py-3 text-sm font-semibold text-[#33373d] transition-colors duration-200 hover:border-[#ed8435] hover:text-[#ed8435]"
              >
