...
const TIPOS_MAQUINARIA = ["Retroexcavadora", "Grúa", "Compactadora", "Bulldozer", "Cargador frontal", "Minicargador", "Vibro compactador", "Pluma hidráulica", "Perforadora", "Otro"];
...
export default function MaquinariaForm() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      const res = await fetch("/api/maquinaria", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
  ...
  if (success) {
    return (
      <div className="rounded-2xl bg-white p-10 text-center shadow-sm">
        ...
        <p className="mt-2 text-sm text-gray-500">Recibimos tu registro de maquinaria. Nuestro equipo te contactará pronto.</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <Section title="1. Datos de la empresa o propietario">
        <div><Label required>Nombre de la empresa o propietario</Label><Input name="nombreEmpresa" placeholder="Ej. Maquinarias del Norte S.A.S." required /></div>
        <div><Label>Descripción del equipo</Label><Textarea name="descripcion" placeholder="Cuéntanos sobre tu maquinaria, capacidad, marca, modelo..." /></div>
        <div><Label>Cantidad de equipos disponibles</Label><Input name="cantidad" type="number" min="1" placeholder="Ej. 3" /></div>
      </Section>

      <Section title="2. Tipos de maquinaria">
        ...
        <div className="flex flex-wrap gap-2">
          {TIPOS_MAQUINARIA.map(t => (
            <button key={t} type="button" onClick={() => toggleTipo(t)}
              className={`rounded-full border px-3 py-1.5 text-xs font-medium transition-colors ${tipos.includes(t) ? "border-[#d97706] bg-[#d97706] text-white" : "border-gray-200 bg-white text-gray-600 hover:border-[#d97706] hover:text-[#d97706]"}`}>
              {t}
            </button>
          ))}
        </div>
      ...
      <Section title="3. Cobertura geográfica">
        ...
        <div><Label>Cobertura adicional</Label><Input name="cobertura" placeholder="Ej. Región Andina, Costa..." /></div>
      </Section>
      ...
      <Section title="4. Información de contacto">
        ...
        <div><Label>Disponibilidad</Label><Input name="disponibilidad" placeholder="Ej. Inmediata, por día o proyecto..." /></div>
      </Section>

      {error && <div className="rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-600">{error}</div>}

      <button type="submit" disabled={loading}
        className="w-full rounded-2xl bg-[#d97706] py-4 text-base font-bold text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-50">
