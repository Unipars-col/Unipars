...
export default function BuscadorMaquinaria() {
  ...
  return (
    <>
      <section className="relative overflow-hidden py-16" style={{ background: "linear-gradient(135deg, #1a1200 0%, #292100 50%, #1a1200 100%)" }}>
        <div className="pointer-events-none absolute inset-0">
          <div className="absolute -left-32 -top-32 h-96 w-96 rounded-full blur-[100px]" style={{ backgroundColor: "rgba(217,119,6,0.12)" }} />
          <div className="absolute -bottom-24 -right-24 h-80 w-80 rounded-full blur-[80px]" style={{ backgroundColor: "rgba(217,119,6,0.10)" }} />
          <div className="absolute left-1/2 top-0 h-px w-3/4 -translate-x-1/2 bg-gradient-to-r from-transparent via-amber-500/30 to-transparent" />
        </div>

        <div className="relative mx-auto max-w-[1440px] px-5 sm:px-8 lg:px-12">
          <div className="mb-10 text-center">
            <span className="mb-4 inline-flex items-center gap-2 rounded-full border border-amber-500/30 bg-amber-500/10 px-4 py-1.5 text-xs font-bold uppercase tracking-[0.3em] text-amber-400">
              <span className="h-1.5 w-1.5 rounded-full bg-amber-400" />
              Alquiler de maquinaria
            </span>
            <h2 className="mt-4 text-4xl font-black leading-tight tracking-tight text-white md:text-5xl">
              Encuentra el equipo
              <span className="block text-amber-400">perfecto para tu proyecto.</span>
            </h2>
            <p className="mx-auto mt-4 max-w-xl text-base text-white/50">
              Maquinaria <strong className="text-white/80">certificada y disponible</strong> en todo el país con o sin operador.
            </p>
          </div>
              ...
              <div className="flex flex-col gap-5 lg:flex-row lg:items-end">
                <div className="flex-1">
                  <label className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/50">
                    <svg className="h-3.5 w-3.5 text-amber-400" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.5c-1.38 0-2.5-1.12-2.5-2.5s1.12-2.5 2.5-2.5 2.5 1.12 2.5 2.5-1.12 2.5-2.5 2.5z"/></svg>
                    Ciudad
                  </label>
                  <select value={ciudad} onChange={(e) => setCiudad(e.target.value)} className="w-full rounded-xl border border-white/10 bg-white/8 px-4 py-4 text-sm font-semibold text-white focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/25">
                    {CIUDADES.map((c) => <option key={c} className="bg-[#1a1200]">{c}</option>)}
                  </select>
                </div>
                ...
                <div className="flex-1">
                  ...
                  <select value={tipo} onChange={(e) => setTipo(e.target.value)} className="w-full rounded-xl border border-white/10 bg-white/8 px-4 py-4 text-sm font-semibold text-white focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/25">
                    {TIPOS.map((t) => <option key={t} className="bg-[#1a1200]">{t}</option>)}
                  </select>
                </div>
                <div className="hidden h-14 w-px bg-white/10 lg:block" />
                <div className="flex-1">
                  <label className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-white/50">
                    <svg className="h-3.5 w-3.5 text-amber-400" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
                    Disponibilidad
                  </label>
                  <select value={disponibilidad} onChange={(e) => setDisponibilidad(e.target.value)} className="w-full rounded-xl border border-white/10 bg-white/8 px-4 py-4 text-sm font-semibold text-white focus:border-amber-500 focus:outline-none focus:ring-2 focus:ring-amber-500/25">
                    {DISPONIBILIDAD.map((d) => <option key={d} className="bg-[#1a1200]">{d}</option>)}
                  </select>
                </div>
                <button onClick={buscar} className="flex-shrink-0 rounded-xl bg-amber-600 px-10 py-4 text-sm font-black uppercase tracking-wider text-white shadow-[0_8px_32px_rgba(217,119,6,0.5)] transition-all hover:-translate-y-1 hover:bg-amber-700 active:translate-y-0">
                  Buscar
